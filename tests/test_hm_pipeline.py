"""
week4/tests/test_hm_pipeline.py
===============================
Unit tests for the H&M-specific LoRA training pipeline.
"""
...
from src.lora.trainers.train_hm_lora import main, run_hm_training
...
class TestHmPipeline:
    """Verify H&M training runner configurations, validation flows, and output targets."""

    def test_run_hm_training_dry_run(self, temp_workspace):
        ...
        assert "--output_name=hm_style" in cmd_str
        ...
        assert final_model_path.name == "hm_style.safetensors"

        # Verify validation output exists
        assert "validation_sample" in res
        assert Path(res["validation_sample"]).exists()

    def test_run_hm_training_raw_ingestion(self, temp_workspace):
        ...
        raw_hm_dir = temp_workspace / "datasets" / "raw_h&m"
        raw_hm_dir.mkdir(parents=True, exist_ok=True)
        
        # Create a raw mock image to ingest
        img = Image.new("RGB", (600, 600), color=(100, 100, 150))
        img_name = "new_hm_basics.png"
        img.save(raw_hm_dir / img_name)
        ...
        manifest_file = temp_workspace / "datasets" / "h&m_manifest.json"
        assert manifest_file.exists()
        
        # Check prepared image exists in the Kohya folder structure
        prepared_dir = temp_workspace / "outputs" / "kohya" / "h&m" / "img"
        sub_folders = list(prepared_dir.glob("10_h&m"))
    ...
    def test_resume_checkpoint_logging(self, temp_workspace):
        ...
        mock_checkpoint_path = temp_workspace / "outputs" / "kohya" / "h&m" / "checkpoint-10"
    ...
    def test_cli_entrypoint(self, temp_workspace):
        ...
        with patch.object(sys, "argv", test_args):
            ...
            final_model = temp_workspace / "outputs" / "hm_style.safetensors"
