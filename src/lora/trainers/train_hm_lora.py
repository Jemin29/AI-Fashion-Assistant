"""
week4/trainers/train_hm_lora.py
===============================
H&M-specific LoRA Fine-Tuning Pipeline.
Automates preprocessing, config formulation, training execution, monitoring,
validation workflow, and checkpoint management to produce hm_style.safetensors.
"""
...
def run_hm_training(
    ...
    """
    Orchestrate the H&M LoRA training workflow.
    ...
    dataset_root : Path or str, optional
        Directory where H&M manifests and images are stored.
    epochs : int
    ...
    """
    logger.info("Initializing H&M-specific LoRA fine-tuning pipeline...")
    ...
    # H&M folder matches brand key "h&m"
    ...
    raw_hm_dir = ds_root / "raw_h&m"
    if not ds_root.joinpath("h&m_manifest.json").exists() and dry_run:
        logger.info(f"Populating mock raw H&M dataset for automated preprocessing at: {raw_hm_dir}")
        raw_hm_dir.mkdir(parents=True, exist_ok=True)
        for i in range(3):
            img = Image.new("RGB", (512, 512), color=(100 + i * 20, 100 + i * 20, 120 + i * 20))
            img.save(raw_hm_dir / f"hm_design_{i}.png")

    # Perform raw files ingestion check
    if raw_hm_dir.exists():
        raw_files = list(raw_hm_dir.glob("*.*"))
        if raw_files:
            logger.info(f"Preprocessing and ingesting {len(raw_files)} raw H&M files...")
    ...
    # Force output name to match target: hm_style
    # Note: KohyaPipeline uses "h&m" brand identifier
    ...
    # Override target output name to yield 'hm_style.safetensors'
    ...
    res = pipeline.run_training(brand="h&m", kohya_script_path=script_path, dry_run=dry_run)

    # 6. Validation Workflow execution
    # Trigger validation image output
    val_output_dir = out_dir / "kohya" / "h&m" / "validation"
    val_output_dir.mkdir(parents=True, exist_ok=True)
    
    if dry_run:
        logger.info("Executing validation workflow: Generating dry-run design samples...")
        val_img = Image.new("RGB", (512, 512), color=(0, 0, 255))
        val_path = val_output_dir / "val_sample_hm_latest.png"
    ...
    model_output_file = out_dir / "hm_style.safetensors"
    
    if res["success"]:
        # Move final weights file to parent output directory as 'hm_style.safetensors'
    ...
    logger.success(f"H&M LoRA training pipeline completed | success={res['success']} | elapsed={elapsed:.2f}s")
    return res


def main() -> int:
    """CLI Entrypoint for the H&M LoRA training pipeline."""
    parser = argparse.ArgumentParser(description="H&M-specific LoRA Fine-Tuning Pipeline.")
